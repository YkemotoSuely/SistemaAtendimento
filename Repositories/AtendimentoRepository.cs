...
namespace SistemaAtendimento.Repositories
{
    public class AtendimentoRepository
    {
        public List<Atendimentos> listar(string termo = "", string condicao = "")
        {
            ...
            using (var conexao = ConexaoDB.GetConexao()) // declarei a variável de conexao
            {
                string sql = @"SELECT a.*, c.nome AS cliente_nome, u.nome AS usuario_nome, 
                            s.nome AS situacao_nome, c.cpf_cnpj FROM atendimentos a 
                            INNER JOIN  clientes c ON c.id = a.cliente_id
                            INNER JOIN usuarios u ON u.id = a.usuario_id
                            INNER JOIN situacao_atendimentos s ON s.id = a.situacao_atendimento_id"; // trecho de código importado do SQL do Banco de Dados
        ...
        public void Inserir(Atendimentos atendimento)
        {
            using (var conexao = ConexaoDB.GetConexao())
            {
                string sql = @"INSERT INTO atendimentos (cliente_id, usuario_id, data_abertura, data_fechamento, observacao,situacao_atendimento_id)" +
                      "VALUES (@cliente_id, @data_abertura, @data_fechamento, @observacao, @situacao_atendimento_id); ";
                
                using (var comando = new SqlCommand(sql, conexao))
                {
                    comando.Parameters.AddWithValue("@cliente_id", atendimento.ClienteId);
                    comando.Parameters.AddWithValue("@susario_id", atendimento.UsuarioId);
                    comando.Parameters.AddWithValue("@data_abertuta", atendimento.DataAbertura);
